...
## Phase 1: Shared Contracts Package ✅ COMPLETE
...
## Phase 2: Eliminate Adapters via Direct Interface Satisfaction ✅ COMPLETE

**Post-audit fix:** Removed dead code in `arrimport/service.go` — the `hub` field (`contracts.Broadcaster`), `WsArrImportProgress` constant, and `hub` constructor parameter were defined but never used after adapter 2f elimination. Removed field, constant, constructor param, and updated call site in `server.go` + test file.
...
## Phase 3: Promote `portalMediaProvisionerAdapter` to a Service ✅ COMPLETE

**Post-audit fixes:** (1) Removed duplicate `provisioner.NewService()` call in `routes.go` that overwrote the registry-tracked instance from `initPortalServices`, which would have caused a dev-mode DB switching regression. (2) Added missing `var _ requests.MediaProvisioner = (*provisioner.Service)(nil)` assertion to `interface_assertions.go`.
...
## Phase 4: Wire Code Generation + Constructor Refactoring ✅ COMPLETE
...
**Stage 4a complete.** All ~45 CONSTRUCTOR-SAFE setters converted to constructor params across 5 parallel subagents (System, Library, Search, Automation+Download, Portal+Metadata). Test files updated. Only `internal/api/server.go` callers remain broken (expected — fixed in Stage 4b).

**Stage 4b complete.** Wire infrastructure created and fully functional:
- `wire.go`: BuildServices injector with ~60 providers, 16 interface bindings, group struct assembly
- `wire_gen.go`: Generated by Wire, compiles and passes all tests
- `providers.go`: ~30 Wire provider functions for types requiring custom construction
- `setters.go`: wireCircularDeps (13 circular setter calls), wireLateBindings (late-binding deps), registerSchedulerTasks, populateRegistry
- `server.go`: Rewritten from ~841 lines to ~230 lines — removed all init* methods
- `routes.go`: Removed inline service creation (AutosearchSettings, RssSyncSettings, ImportSettings, RequestSearcher, AdminSettings) — now Wire-provided
- `service_groups.go`: Added ServiceContainer, AutosearchSettings, FeedFetcher fields
- Build passes, all tests pass with race detector, zero lint issues.
...
## Phase 5: Reflection-Based Service Registry ✅ COMPLETE
...
**Post-audit fixes:** (1) Added startup `Validate()` call in `NewServer` (after `wireCircularDeps`) so unregistered switchable services are caught at boot, not silently at dev mode toggle time. (2) Added 4 tests in `switchable_test.go`: `TestAllSwitchableServicesRegistered` (integration), `TestSwitchableServices_UpdateAll_NoPanic`, `TestSwitchableServices_Validate_DetectsNilFields` (unit), `TestSwitchableServices_Validate_SkipsOptional` (unit).

### `SwitchableServices` struct (as implemented in `switchable.go`):

40 fields total (29 `db`, 10 `queries`, 1 `queries,optional`). Key deviations from original plan:

- `NetworkLogoStore *metadata.SQLNetworkLogoStore` instead of `Metadata *metadata.Service` — only the logo store subcomponent has `SetDB`
- `StatusTracker`, `LibraryChecker`, `Watchers` use `switchable:"db"` (not `queries`) — matches actual `SetDB(*sql.DB)` signatures
- 6 additional services registered beyond plan: `Preferences`, `LibraryManager`, `AdminLibraryChecker`, `AutoApprove`, `Auth`, `Passkey`
- `Passkey` tagged `switchable:"queries,optional"` — may be nil when WebAuthn is unavailable

### Tag matching uses `strings.HasPrefix` (improvement over plan):

The plan specified exact `case "db"` / `case "queries"` matching. Implementation uses `strings.HasPrefix(tag, "db")` / `strings.HasPrefix(tag, "queries")` to support compound tags like `"queries,optional"`.

### `Validate()` supports `optional` tag modifier:

Fields tagged with `"optional"` (e.g., `switchable:"queries,optional"`) are skipped by `Validate()`. This allows services that may legitimately be nil (like `Passkey` when WebAuthn is disabled) without triggering false validation failures.

### Startup validation in `NewServer`:

```go
// In NewServer, after wireCircularDeps:
...
### Why this works:
- Adding a field with a `switchable` tag but forgetting to assign it → caught by `Validate()` at startup (fatal)
...
### Tests (`switchable_test.go`):

| Test | Type | What it verifies |
|------|------|-----------------|
| `TestAllSwitchableServicesRegistered` | Integration | Wire + circular deps populate all 40 fields, `Validate()` passes |
| `TestSwitchableServices_UpdateAll_NoPanic` | Integration | `UpdateAll` completes without panic (catches missing `SetDB` methods) |
| `TestSwitchableServices_Validate_DetectsNilFields` | Unit | Empty struct reports all unset required fields |
| `TestSwitchableServices_Validate_SkipsOptional` | Integration | `Passkey` (tagged `optional`) can be nil without validation failure |

### Performance note:
Reflection-based `UpdateAll` is ~10µs for 40 fields. Dev mode toggle happens at most once per user action. Performance is irrelevant here.

**Files:** New `switchable.go` replaces `service_registry.go`. New `switchable_test.go` with 4 tests. Startup validation added to `server.go`. Wire assigns to `c.Switchable.X` fields as part of the generated code.

---

## Phase 6: Dev Mode Manager Extraction ✅ COMPLETE
