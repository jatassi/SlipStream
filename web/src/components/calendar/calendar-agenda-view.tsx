...
export function CalendarAgendaView({ events, loading }: CalendarAgendaViewProps) {
  ...
  if (groupedEvents.length === 0) {
    return <div className="text-muted-foreground flex h-64 items-center justify-center">No upcoming events in this date range</div>
  }
  ...
  return (
    <div className="space-y-6">
      {groupedEvents.map(([dateStr, dayEvents]) => {
        ...
        return (
          <div key={dateStr}>
            <div className={cn('bg-background/95 sticky top-0 z-10 mb-2 py-2 supports-[backdrop-filter]:backdrop-blur-sm flex items-center gap-3')}>
              <div className={cn('flex h-14 w-14 flex-col items-center justify-center rounded-lg', isCurrentDay && 'bg-primary text-primary-foreground', isPastDay && 'bg-muted text-muted-foreground', isFutureDay && !isCurrentDay && 'bg-secondary')}>
                <span className="text-xs uppercase">{format(date, 'EEE')}</span>
                <span className="text-xl font-bold">{format(date, 'd')}</span>
              </div>
              <div>
                <div className={cn('font-semibold', isCurrentDay && 'text-primary')}>{isCurrentDay ? 'Today' : format(date, 'EEEE')}</div>
                <div className="text-muted-foreground text-sm">{format(date, 'MMMM d, yyyy')}</div>
              </div>
              <div className="text-muted-foreground ml-auto text-sm">{dayEvents.length} {dayEvents.length === 1 ? 'event' : 'events'}</div>
            </div>
            <div className="space-y-2 pl-[70px]">
              {dayEvents.map((event) => (
