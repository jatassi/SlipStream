...
function Progress({ className, children, value, variant = 'default', ...props }: ProgressProps) {
  return (
...
function ProgressIndicator({ className, ...props }: ProgressPrimitive.Indicator.Props) {
  return (
    <ProgressPrimitive.Indicator
      ...
      className={cn('bg-primary h-full transition-[width,inset-inline-start] duration-700 ease-linear', className)}
      {...props}
    />
  )
}
