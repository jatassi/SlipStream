...
export function DryRunModal({
  ...
  const handleExecute = () => {
    ...
    executeMutation.mutate(overrides.length > 0 ? { overrides } : undefined, {
      onSuccess: (result) => {
        if (result.success) {
          toast.success(
            `Multi-version mode enabled! ${result.filesAssigned} files assigned to slots.`,
          )
          setConfirmModalOpen(false)
          onOpenChange(false)
          onMigrationComplete()
  ...
  const allFilesAccountedFor = editedPreview
    ? editedPreview.summary.filesWithSlots + ignoredFileIds.size ===
      editedPreview.summary.totalFiles
    : false
      ...
      <DialogContent className="flex max-h-[90vh] flex-col sm:max-w-4xl">
        ...
        <DialogFooter className="mt-2 shrink-0">
          ...
          <Button
            ...
            disabled={
              isLoading || isExecuting || !editedPreview || isDebugData || !allFilesAccountedFor
            }
          >
          ...
          <Alert className="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/50">
            ...
            <AlertDescription className="text-blue-700 dark:text-blue-300">
              <ul className="mt-2 list-inside list-disc space-y-1">
                ...
                <li>
                  You can disable multi-version mode later, but file assignments will be preserved
                </li>
              </ul>
            </AlertDescription>
          </Alert>
