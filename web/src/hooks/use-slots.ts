...
export function useUpdateMultiVersionSettings() {
  ...
  return useMutation({
    ...
    onSuccess: (settings: MultiVersionSettings) => {
      ...
      void queryClient.setQueryData(slotsKeys.settings(), settings)
    },
  })
}
...
export function useUpdateSlot() {
  ...
  return useMutation({
    ...
    onSuccess: (slot: Slot) => {
      void queryClient.invalidateQueries({ queryKey: slotsKeys.all })
      void queryClient.setQueryData(slotsKeys.detail(slot.id), slot)
    },
  })
}
...
export function useSetMovieSlotMonitored() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { movieId }) => {
      void queryClient.invalidateQueries({ queryKey: slotsKeys.movieStatus(movieId) })
      void queryClient.invalidateQueries({ queryKey: slotsKeys.movieAssignments(movieId) })
    },
  })
}
...
export function useSetEpisodeSlotMonitored() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { episodeId }) => {
      void queryClient.invalidateQueries({ queryKey: slotsKeys.episodeStatus(episodeId) })
      void queryClient.invalidateQueries({ queryKey: slotsKeys.episodeAssignments(episodeId) })
    },
  })
}
...
export function useExecuteMigration() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: slotsKeys.all })
      void queryClient.invalidateQueries({ queryKey: slotsKeys.settings() })
    },
  })
}
