...
export function useScanLibrary() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      // Invalidate scan statuses to trigger refresh
      queryClient.invalidateQueries({ queryKey: libraryKeys.scans() })
    },
    onSettled: () => {
      // Invalidate movies and series after scan starts
      // The actual data will be refreshed via WebSocket or manual refresh
      setTimeout(() => {
