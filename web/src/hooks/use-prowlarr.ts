...
export function useUpdateProwlarrConfig() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: prowlarrKeys.all })
    },
  })
}
...
export function useRefreshProwlarr() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: prowlarrKeys.indexers() })
      void queryClient.invalidateQueries({ queryKey: prowlarrKeys.capabilities() })
      void queryClient.invalidateQueries({ queryKey: prowlarrKeys.status() })
    },
  })
}
